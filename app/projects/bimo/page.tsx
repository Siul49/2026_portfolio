    ...
    <article className="min-h-screen pt-32 pb-20 px-8 max-w-5xl mx-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-12">
        <div className="md:col-span-4">
           <h3 className="text-lg font-bold mb-4 font-serif text-[var(--color-deep-navy)]">The Concept</h3>
        ...
        <div className="md:col-span-8">
           <div className="bg-gray-50 p-8 rounded-lg border border-[var(--color-grid-line)] mb-8">
              <h4 className="font-mono text-xs text-[var(--color-serene-blue)] mb-4">/// AI PIPELINE FLOW</h4>
              <div className="space-y-6">
                 <div className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-[var(--color-deep-navy)] text-white flex items-center justify-center font-mono text-xs mt-1">1</div>
                    <div>
                        <h5 className="font-bold text-[var(--color-deep-navy)]">Image Ingestion</h5>
                        <p className="font-light text-sm mt-1">사용자가 업로드한 탑승권 이미지를 전처리(Preprocessing)하여 노이즈를 제거합니다.</p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-[var(--color-serene-blue)] text-white flex items-center justify-center font-mono text-xs mt-1">2</div>
                    <div>
                        <h5 className="font-bold text-[var(--color-deep-navy)]">Multimodal Analysis (Gemini)</h5>
                        <p className="font-light text-sm mt-1">Gemini Vision API에 이미지를 주입하여 <code>Flight Number</code>, <code>Date</code>, <code>Seat Class</code> 등 핵심 메타데이터를 구조화된 JSON으로 추출합니다.</p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-gray-400 text-white flex items-center justify-center font-mono text-xs mt-1">3</div>
                    <div>
                        <h5 className="font-bold text-[var(--color-deep-navy)]">Contextual Guide</h5>
                        <p className="font-light text-sm mt-1">추출된 데이터를 기반으로 기내식 정보, 좌석 꿀팁, 시차 적응 가이드를 생성하여 제공합니다.</p>
                    </div>
                 </div>
              </div>
           </div>
